//! Helpers for generating mock telemetry data.
...
/// Generates mock telemetry records and appends them to a JSONL file.
pub struct MockDataGenerator;

impl MockDataGenerator {
    /// Appends `count` randomly generated telemetry records to `path`.
    ///
    /// # Examples
    ///
    /// ```rust,no_run
    /// # fn main() -> anyhow::Result<()> {
    /// use rustpulse::infra::mock_telemetry::MockDataGenerator;
    ///
    /// let path = std::env::temp_dir().join("telemetry.jsonl");
    /// MockDataGenerator::generate_mock_data(&path, 10)?;
    /// # Ok(())
    /// # }
    /// ```
